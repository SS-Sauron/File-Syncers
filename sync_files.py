...
Production-grade one-way file synchronisation.

Compares a flat source directory against a project tree using SHA-256
content hashing.  Every file about to be overwritten receives a
timestamped .bak backup first.  Provides dry-run preview, unified diff
summary, move mode, extension filtering, and structured logging.

All 19 issues identified in the v1.0 code review have been addressed.
Changes by priority:

  P1 – Active bugs
    #1  --move no longer copies identical files. Hash equality is always
        honoured; the source is still queued for deletion when --move is
        set, but no redundant backup or copy is performed.
    #2  --show-diff now reports the FIRST changed line, not the last.
        The original loop overwrote `location` on every @@ marker.
    #3  A summary line is always emitted after every run (dry-run or
        live).  The original code silenced it behind `if not dry_run`.
    #7  Exits with code 1 when errors occur so shell scripts, Makefiles,
        and CI pipelines can detect a failed sync.

  P2 – Architectural risks
    #5  Pre-scan warns before any writes when one source file would
        overwrite multiple project destinations (filename-only matching).
    #16 Aborts at startup if --backup-path resolves to a directory
        inside --project-path (re-sync loop prevention).

  P3 – Correctness & efficiency
    #4  ChangeRecord objects are only allocated when --show-diff is
        active; the list is never built speculatively.
    #6  Source-file SHA-256 hashes are cached per run. Each source file
        is read at most once, regardless of how many project files share
        its name.
    #8  Backup filenames carry microsecond precision (%f), eliminating
        the original second-level collision risk.
    #14 Binary files are probed via null-byte detection and excluded from
        diff processing entirely.

  P4 – Code quality
    #9  Full pathlib.Path migration throughout via _compat_walk() wrapper
        (Python 3.8+ compatible; Path.walk() requires 3.12).
    #10 All output routes through logging. The one intentional print()
        for the diff summary table is documented explicitly.
    #11 SyncConfig dataclass replaces the fragile 7-positional-argument
        function signature.
    #12 PEP 484 type hints on every function and dataclass field.
    #13 --ignore-dirs uses nargs='+' (was '*') so a bare flag cannot
        silently clear .git and __pycache__ from the ignore list.

  P5 – New features
    #15 Flat-source assumption is explicit in docs and surfaced via INFO
        log so users know sub-directory files are intentionally excluded.
    #17 --extensions flag restricts sync to specific file suffixes.
    #18 --log-file appends the full log to a file alongside the console.
    #19 --version flag added.
"""
